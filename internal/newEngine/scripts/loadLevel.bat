...
set string1=%~1
call newEngine\scripts\checkString.bat "!string1!" "\ ." allowLetters allowNumbers
	...
	if "%%a"=="tile" (
		...
		for /f "tokens=1-2 delims=: " %%e in (newEngineProject\tiles\%%b) do (
			if "%%e"=="collisionGroup" (
				...
				if "!num1!"=="-1" (
					set /a num1=-1
					for /l %%g in (10,-1,0) do if NOT defined lcg_%%g set /a num1=%%g
					if "!num1!"=="10" %rhe% TOO_MANY_COLLISION_GROUPS
				)
				if "!num1!"=="-1" exit /b 1
				set lcg_!num1!=%%f
